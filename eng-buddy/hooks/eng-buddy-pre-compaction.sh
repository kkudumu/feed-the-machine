...
#
# FILES:
#   ~/.claude/eng-buddy/.session-active     - session gate (set by session-manager.sh)
#   ~/.claude/eng-buddy/.last-flush-tokens  - token count at last flush (cooldown)

SESSION_ACTIVE="$HOME/.claude/eng-buddy/.session-active"
LAST_FLUSH_FILE="$HOME/.claude/eng-buddy/.last-flush-tokens"
...
# Claude Code prepends this as <user-prompt-submit-hook> context
...
echo "1. Append unsaved session progress to ~/.claude/eng-buddy/daily/${TODAY}.md (new requests, completions, blockers, context switches)"
echo "2. Update ~/.claude/eng-buddy/tasks/active-tasks.md with any changed task statuses"
