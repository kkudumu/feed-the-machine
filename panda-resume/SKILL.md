...
```
Resuming panda-{skill} session from {timestamp}
Project: {project_dir}
...
**"Start fresh" / "Never mind"** — archive the state file (see Step 6) and tell the user: "State archived. You can start a fresh session with /panda-{skill}."

---

## Step 5: Invoke the Appropriate Panda Skill

Invoke the panda skill with the full saved context injected so it picks up exactly where it left off, not from the beginning. Construct a context preamble that tells the skill its position.

Read `../panda-pause/references/protocols/SKILL-RESTORE-PROTOCOLS.md` for the exact per-skill state fields to inject and restoration instructions for each skill (brainstorm, executor, debug, council, audit).
...
- executor: `/panda-executor {plan_path}`
- debug: `/panda-debug`
...
Archive immediately after invocation — not after the full session completes. The state has been consumed. If the user needs to pause again, panda-pause will create a new STATE.md from the now-continued session.

---

## Edge Cases

**Multiple state files** — If both `STATE.md` and `STATE-panda-brainstorm.md` exist, ask the user which one to resume. List them with skill type and timestamp.
...
**Nothing to resume** — If `~/.claude/panda-state/` doesn't exist or contains no STATE.md:
```
