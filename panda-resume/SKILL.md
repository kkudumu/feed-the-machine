---
name: panda-resume
description: Resume a previously paused panda skill session from saved state. Use when user says "resume", "continue where we left off", "panda resume", "pick up", or starts a new conversation wanting to continue previous panda work.
---

## Events

### Emits
- `session_resumed` — when a saved session state has been validated and the appropriate panda skill is re-invoked with restored context
- `task_completed` — when the resume workflow finishes (skill re-invoked and state file archived)

### Listens To
(none — panda-resume is explicitly invoked by the user and does not respond to events)

# Panda Resume — Session State Restoration

Read a saved panda session state file and seamlessly continue the workflow in a fresh conversation. The user should feel like they never left — same context, same decisions, same progress, picking up at exactly the right step.

## Why This Exists

Panda skills are long-running, multi-phase workflows. A brainstorm session might span 10+ turns of research and questioning. An executor session might be mid-way through wave 2 of a 5-wave plan. A debug war room might have 4 investigation agents completed and a solver mid-attempt. When these sessions end — whether by choice, crash, or context exhaustion — the work shouldn't be lost. This skill reads the state file that panda-pause saved and reconstructs the session so the appropriate panda skill can continue exactly where it stopped.

## Step 1: Read the State File

Read `~/.claude/panda-state/STATE.md`.

If the file doesn't exist:
```
No saved panda session found at ~/.claude/panda-state/STATE.md

To save a session mid-workflow, use /panda-pause during any active panda skill.
```
Stop here.
...
|-------|----------|---------|
| `skill` | Yes | Which panda skill to resume (brainstorm, executor, debug, council, audit) |
...
### For panda-brainstorm:
```
Resuming panda-brainstorm session from {timestamp}
Project: {project_dir}
...
### For panda-executor:
```
Resuming panda-executor session from {timestamp}
Project: {project_dir}
Plan: {plan_path}
...
### For panda-debug:
```
Resuming panda-debug war room from {timestamp}
Project: {project_dir}
...
### For panda-council:
```
Resuming panda-council session from {timestamp}
Project: {project_dir}
...
### For panda-audit:
```
Resuming panda-audit session from {timestamp}
Project: {project_dir}
...
Archive the state file (see Step 7) and tell the user: "State archived. You can start a fresh session with /panda-{skill}."

## Step 6: Invoke the Appropriate Panda Skill

This is the critical step. You need to invoke the panda skill with the full saved context injected so it picks up exactly where it left off, not from the beginning.
...
Invoke the panda-brainstorm skill with the following context preamble:
...
Then invoke `/panda-brainstorm` via the Skill tool. The brainstorm skill will see this context and should continue from the specified point — running the next research sprint, asking the next question, or generating the next section of the plan.
...
Then invoke `/panda-executor {plan_path}` via the Skill tool.
...
Then invoke `/panda-debug` via the Skill tool.
...
Then invoke `/panda-council` via the Skill tool.
...
Then invoke `/panda-audit` via the Skill tool.
...
```bash
mkdir -p ~/.claude/panda-state/archive
mv ~/.claude/panda-state/STATE.md ~/.claude/panda-state/archive/STATE-$(date +%Y%m%d-%H%M%S).md
```

Do this immediately after the skill is invoked, not after the full session completes. The state has been consumed — if the user needs to pause again, panda-pause will create a new STATE.md from the current (now-continued) session.

## Edge Cases

### State file references a skill that isn't installed
If the state says `skill: panda-debug` but panda-debug isn't in the skills directory, tell the user: "The saved session requires panda-debug but that skill isn't available. Install it and try again."
...
If both `STATE.md` and `STATE-panda-brainstorm.md` exist (from a session where multiple skills were paused), ask the user which one to resume. List them with their skill type and timestamp.
...
### The user runs /panda-resume but there's nothing to resume
If `~/.claude/panda-state/` doesn't exist or contains no STATE.md:
```
No saved panda session found.

To save a session for later:
1. Start any panda skill (/panda-brainstorm, /panda-executor, /panda-debug, /panda-council, /panda-audit)
2. When you need to stop, use /panda-pause
3. In a new conversation, use /panda-resume to continue
```

### Archived states
If the user asks "do I have any old sessions?" or "what sessions have I saved?", check the archive:
```bash
ls -la ~/.claude/panda-state/archive/
```
