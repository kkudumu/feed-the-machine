...
|-------|----------|---------|
| `skill` | Yes | Which panda skill to resume |
...
If the state file has a `panda_version` field, compare it to the installed skill version.

- **Same version**: Pass.
- **Minor version difference**: Warn. "This state was saved with an older version of panda. The workflow may behave slightly differently, but resumption should work."
- **Major version difference**: Block with explanation. "This state was saved with panda v{N}, but the installed version is v{M}. Major version changes may have altered skill structure. Resuming could produce unexpected behavior. Recommend starting fresh."
...
If the state says `skill: panda-debug` but that skill isn't in the skills directory:

"The saved session requires panda-{skill} but that skill isn't available. Install it and try again."
