...
Before dispatching any agents, read `~/.codex/ftm-config.yml`:
...
2. Read `~/.codex/ftm-state/blackboard/experiences/index.json` — filter by task_type "feature"/"investigation"
3. Load top 3-5 matching experience files for past brainstorm lessons
4. Read `~/.codex/ftm-state/blackboard/patterns.json` — check execution_patterns and user_behavior
...
EVERY TURN (after initial intake):
  ...
  4. ASK ONE QUESTION — ask one concise blocking question in chat (or `request_user_input` only in Plan mode)
...
**Use the Codex-native question flow for all questions.** In Default mode, ask a concise blocking question directly in chat with 2-4 inline options. In Plan mode, you may use `request_user_input` for the same structure.

**Batching rules:** in Codex, only batch questions when you are in Plan mode and `request_user_input` is available. In Default mode, ask one blocking question at a time.
...
Run a repo scan first. Prefer local parallel reads; if and only if the user explicitly asked for sub-agents, you may spawn an `explorer` agent:
```
...
Every turn, read `references/agent-prompts.md` and run **3 parallel research lanes**. Use `multi_tool_use.parallel` plus local tools by default; only use `spawn_agent` if the user explicitly asked for sub-agents. Each lane gets:
...
## Step 4: Ask One Blocking Question

In Codex Default mode, ask the next blocking question directly in chat. If the session is in Plan mode, `request_user_input` is the preferred structured UI.
...
**Batch independent questions only in Plan mode.** In Default mode, send only the single blocking question and keep the rest in your internal queue.
...
**Example blocking question format:**
```json
{
  "questions": [
    {
...
1. `~/.codex/ftm-state/blackboard/context.json`:
...
2. Write experience file to `~/.codex/ftm-state/blackboard/experiences/YYYY-MM-DD_task-slug.json`:
...
- config: `~/.codex/ftm-config.yml` | optional | model profile for planning agents
...
- reference: `~/.codex/ftm-state/blackboard/context.json` | optional | session state and active constraints
- reference: `~/.codex/ftm-state/blackboard/experiences/index.json` | optional | past brainstorm lessons
- reference: `~/.codex/ftm-state/blackboard/patterns.json` | optional | execution and user behavior patterns

## Risk

- level: low_write
- scope: writes plan documents to ~/.codex/plans/; writes blackboard context and experience files; does not modify project source code
