...
Read `~/.codex/ftm-config.yml`. If it does not exist, create it with the default configuration (balanced profile active, all defaults as shown in the schema above). Use the file-creator pattern or Write tool to create the file.
...
3. **Save changes**: Write the updated YAML back to `~/.codex/ftm-config.yml`.
...
3. If confirmed, write the default configuration to `~/.codex/ftm-config.yml`.
...
All ftm skills should read `~/.codex/ftm-config.yml` at the start of execution to determine which model to use when spawning agents.

### Reading the Config

At the beginning of any ftm skill execution:

1. Read `~/.codex/ftm-config.yml`.
...
The skill will create `~/.codex/ftm-config.yml` with default settings automatically. No action needed.

### Invalid model name
Only `opus`, `sonnet`, `haiku`, and `inherit` are valid. The skill will reject other values and show the valid options.

### Config file is malformed
If the YAML cannot be parsed, the skill will back up the broken file as `~/.codex/ftm-config.yml.bak` and create a fresh default config.
...
- config: `~/.codex/ftm-config.yml` | optional | main config file (created with defaults if missing)

## Risk

- level: low_write
- scope: reads and writes ~/.codex/ftm-config.yml only; backs up malformed config to ftm-config.yml.bak before overwriting; no project files touched
- rollback: restore from ~/.codex/ftm-config.yml.bak or delete the file to reset to defaults on next invocation
