...
| **Playbook** | `~/.codex/eng-buddy/playbooks/{name}.json` | JSON with exact tool params | eng-buddy playbook engine |
...
1. **Blackboard context**: `~/.codex/ftm-state/blackboard/context.json`
   - Extract: `current_task`, `recent_decisions`, `active_constraints`
2. **Today's daily log**: `~/.codex/eng-buddy/daily/{today}.md`
   - Extract: completed items, tool calls, blockers encountered, lessons learned
3. **Recent experiences**: `~/.codex/ftm-state/blackboard/experiences/index.json`
...
5. **Existing playbooks**: `ls ~/.codex/eng-buddy/playbooks/` — check for existing playbook
...
### Step 5: Write the Playbook (`~/.codex/eng-buddy/playbooks/{name}.json`)
...
Write to `~/.codex/ftm-state/blackboard/experiences/{name}-capture.json`:
...
```json
{
  ...
  "files_touched": [
    ...
    "~/.codex/eng-buddy/playbooks/{name}.json",
...
Update `~/.codex/ftm-state/blackboard/context.json`:
...
Written to:
  ...
  Playbook:  ~/.codex/eng-buddy/playbooks/{name}.json
...
### eng-buddy
eng-buddy's playbook engine reads from `~/.codex/eng-buddy/playbooks/`. Captured playbooks show up on the dashboard's Playbooks tab.
...
- reference: `~/.codex/ftm-state/blackboard/context.json` | required | current task and recent decisions for pattern extraction
- reference: `~/.codex/eng-buddy/daily/{today}.md` | optional | daily log for completed items and tool calls
- reference: `~/.codex/ftm-state/blackboard/experiences/index.json` | required | today's session experiences
- reference: `~/.ftm/routines/` | optional | check for existing routines before creating new one
- reference: `~/.codex/eng-buddy/playbooks/` | optional | check for existing playbooks before creating
- reference: `~/Documents/Code/panda/docs/playbooks/` | optional | check for existing reference docs before creating

## Risk

- level: low_write
- scope: writes YAML routine to ~/.ftm/routines/, JSON playbook to ~/.codex/eng-buddy/playbooks/, and Markdown reference doc to ~/Documents/Code/panda/docs/playbooks/; writes experience to blackboard; does not modify project source code
