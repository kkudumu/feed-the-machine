...
2. Read `~/.codex/ftm-state/blackboard/experiences/index.json` — filter entries by tags matching "validation", "codex", or "review"
3. Load top 3-5 matching experience files for patterns in what Codex commonly flags or auto-fixes
4. Read `~/.codex/ftm-state/blackboard/patterns.json` — check recurring_issues for common validation failures and execution_patterns for typical fix types
...
**Flags reference:**
- `--yolo` — No sandbox, no approval prompts; Codex is the outer sandbox
...
2. Write an experience file to `~/.codex/ftm-state/blackboard/experiences/YYYY-MM-DD_task-slug.json` capturing gate mode, verdict, tests formed/passed, fixes applied, and any INTENT.md conflicts found
...
- reference: `~/.codex/ftm-state/blackboard/experiences/index.json` | optional | prior validation patterns
