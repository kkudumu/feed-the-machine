...
Codex-native mode does not require external model CLIs.
Use local tools and Codex reasoning lanes by default. External CLIs, if they appear later in this file, are legacy reference only.
...
Show the user the framed prompt before proceeding and wait for confirmation or edits.
...
Run the three council lanes in parallel. Prefer local tool work; only use `spawn_agent` if the user explicitly asked for sub-agents or parallel agent work.
...
2. Write an experience file to `~/.codex/ftm-state/blackboard/experiences/YYYY-MM-DD_task-slug.json` capturing decision domain, verdict, round reached, dissent summary, and whether the verdict held up
