...
2. Read `~/.codex/ftm-state/blackboard/experiences/index.json` — filter entries by task_type="bug" and tags matching the current error domain
3. Load top 3-5 matching experience files for known fixes and failed approaches
4. Read `~/.codex/ftm-state/blackboard/patterns.json` — check recurring_issues for matching symptoms and codebase_insights for relevant file patterns
...
process being fixed (e.g., debugging Codex from within Codex),
...
5. **ftm-browse ($PB) for UI verification**: If ftm-browse is
   ...
   ```bash
   PB="$CODEX_HOME/skills/ftm-browse/bin/ftm-browse"
   ```
   ...
   **Graceful fallback**: If the binary does NOT exist at
   `$CODEX_HOME/skills/ftm-browse/bin/ftm-browse`, fall back to
...
2. Write an experience file to `~/.codex/ftm-state/blackboard/experiences/YYYY-MM-DD_task-slug.json` capturing root cause, hypotheses tested, fix approach, and what to check first next time
...
Live verification for this bug would be: launch Codex, wait for the statusline to render, capture a screenshot, confirm the doom head is visible. That's what the Reviewer must do for visual bugs.

## Requirements

- config: `~/.codex/ftm-config.yml` | optional | model profiles for investigation agents
...
- reference: `~/.codex/ftm-state/blackboard/context.json` | optional | session state
- reference: `~/.codex/ftm-state/blackboard/experiences/index.json` | optional | past bug fixes and known issues
- reference: `~/.codex/ftm-state/blackboard/patterns.json` | optional | recurring failure patterns
