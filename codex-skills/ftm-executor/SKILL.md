...
1. Read `~/.codex/ftm-state/blackboard/context.json` — check current_task, recent_decisions, active_constraints
2. Read `~/.codex/ftm-state/blackboard/experiences/index.json` — filter entries by task_type matching plan tasks and tags overlapping with the plan domain
3. Load top 3-5 matching experience files for relevant lessons on agent performance and timing
4. Read `~/.codex/ftm-state/blackboard/patterns.json` — check execution_patterns for agent performance and timing accuracy patterns
...
1. Was a plan path provided? (e.g., the user said "execute ~/.codex/plans/foo.md" or ftm-mind routed with a plan reference)
...
3. If no plan path was provided, **do not start coding**. Instead:
   ...
   e. Only after approval, save the plan to `~/.codex/plans/` and proceed to Phase 0.5.
...
Read `~/.codex/ftm-config.yml` to determine which models to use for agent dispatch. If the file doesn't exist, use balanced defaults:
...
3. **STYLE.md** (project root) — If missing, copy from `~/.codex/skills/ftm-executor/references/STYLE-TEMPLATE.md` into the project root.
...
If `progress_tracking` is enabled in `~/.codex/ftm-config.yml` (default: true), create a `PROGRESS.md` file in the project root that gets updated after every significant event. This gives visibility into long-running executions without interrupting them.
...
5. **Visual smoke test (optional)** — If the project has a running dev server (detected via `lsof -i :3000` or `lsof -i :5173` or configured in plan metadata as `dev_server_url`), run:
   ...
   Where `$PB` is `$CODEX_HOME/skills/ftm-browse/bin/ftm-browse`.

   **Graceful degradation**: If ftm-browse binary is not installed at `$CODEX_HOME/skills/ftm-browse/bin/ftm-browse`, skip visual checks with a note: "Visual smoke test skipped — ftm-browse not installed." Do not fail the task.
...
The retro runs automatically — do not ask the user whether to run it. The report is saved to `~/.codex/ftm-retros/` and a one-line summary of the overall score (X/50) is shown to the user before presenting the branch finishing options.
...
1. Update `~/.codex/ftm-state/blackboard/context.json`:
...
2. Write an experience file to `~/.codex/ftm-state/blackboard/experiences/YYYY-MM-DD_task-slug.json` capturing task_type, agent team used, wave count, audit outcomes, and lessons learned
3. Update `~/.codex/ftm-state/blackboard/experiences/index.json` with the new entry
4. Emit `task_completed` event

## Requirements

- tool: `git` | required | worktree creation, branch management, commit operations
- config: `~/.codex/ftm-config.yml` | optional | model profiles, max_parallel_agents, auto_audit, progress_tracking
- reference: `~/.codex/plans/` | optional | plan documents for execution
- tool: `node` | optional | project setup commands in worktrees
- reference: `~/.codex/skills/ftm-executor/references/STYLE-TEMPLATE.md` | optional | STYLE.md bootstrap template
...
- condition: ftm-browse not installed at $CODEX_HOME/skills/ftm-browse/bin/ftm-browse | action: skip visual smoke test checks, log "Visual smoke test skipped — ftm-browse not installed"
