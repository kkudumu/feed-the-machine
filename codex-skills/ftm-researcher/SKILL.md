...
1. Load `~/.codex/ftm-blackboard/context.json`
...
4. Load `~/.codex/ftm-blackboard/patterns.json` for recurring research patterns
...
```
PHASE 0: REPO SCAN
  Run a silent repo scan with local tools first. If the user explicitly asked for sub-agents, an `explorer` agent may assist.
...
PHASE 2: WAVE 1
  - Cover up to 7 finder lenses in parallel. Prefer local tool parallelism; only dispatch sub-agents when explicitly authorized by the user. Each research lane should have:
...
- "dig deeper on finding #N" / "more on #N" → run a targeted follow-up research pass on that finding; use sub-agents only if explicitly authorized
- "I disagree with X" / "I think X is wrong because Y" → run a counter-evidence pass and update findings
- "focus on [angle]" / "what about the security angle" → reshape subtopics with new weighting and rerun the research lanes
- "council finding #N" / "get more opinions on #N" → route specific claim to ftm-council
- "more on [lane]'s findings" → rerun that research lane with a broader query
- "compare A vs B" → run a comparison pass with both findings as context
...
Deep mode only. Routes top claims through `ftm-council` for Codex-native multi-lens adversarial review.
...
1. Update `~/.codex/ftm-blackboard/context.json` with research session summary
2. Write experience file: `~/.codex/ftm-blackboard/experiences/research-[timestamp].json`
   - Fields: query, mode, findings_count, top_consensus_claims, source_diversity, duration_ms
3. Update `~/.codex/ftm-blackboard/index.json` with new experience entry
...
- config: `~/.codex/ftm-config.yml` | optional | planning and review model profiles, per_skill_overrides.ftm-researcher agent cap
...
- reference: `~/.codex/ftm-blackboard/context.json` | optional | session state
- reference: `~/.codex/ftm-blackboard/patterns.json` | optional | recurring research patterns
