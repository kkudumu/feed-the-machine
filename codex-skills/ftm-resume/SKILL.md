...
```bash
mkdir -p ~/.codex/ftm-state/archive
mv ~/.codex/ftm-state/STATE.md ~/.codex/ftm-state/archive/STATE-$(date +%Y%m%d-%H%M%S).md
```
...
If `~/.codex/ftm-state/` doesn't exist or contains no STATE.md:
```
...
- reference: `~/.codex/ftm-state/STATE.md` | required | saved session state file from ftm-pause
...
- reference: `~/.codex/ftm-state/archive/` | optional | archived prior state files
- tool: `git` | optional | checking git state drift since session was paused

## Risk

- level: low_write
- scope: archives STATE.md by moving it to ~/.codex/ftm-state/archive/; invokes the target ftm skill with restored context; does not modify project source files
