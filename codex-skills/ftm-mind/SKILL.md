...
- `$ftm-executor ~/.codex/plans/foo.md` -> route to `ftm-executor`
...
- `~/.codex/ftm-state/blackboard/context.json`
...
4. **Write capabilities.json** to `~/.codex/ftm-state/blackboard/capabilities.json`:
   ```json
   {
...
- `~/.codex/ftm-state/blackboard/context.json`
- `~/.codex/ftm-state/blackboard/experiences/index.json`
- `~/.codex/ftm-state/blackboard/patterns.json`
...
1. Read `~/.codex/ftm-state/blackboard/experiences/index.json`
2. Parse `entries`
...
7. Load the top 3-5 matching experience files from:
   - `~/.codex/ftm-state/blackboard/experiences/{filename}`
...
- `~/.codex/skills/ftm-mind/references/mcp-inventory.md`
...
Read `~/.codex/ftm-config.yml` field `execution.approval_mode`. This controls whether the user sees and approves the plan before execution begins.
...
**Drafts folder**: `.ftm-drafts/` in the project root (or `~/.codex/ftm-drafts/` if no project context).
...
3. Write an experience file to `~/.codex/ftm-state/blackboard/experiences/YYYY-MM-DD_task-slug.json`
4. Update `~/.codex/ftm-state/blackboard/experiences/index.json` with the new entry
...
| `execute ~/.codex/plans/foo.md` | explicit plan path and execution ask | route to `ftm-executor` |
...
1. Update `context.json` — set `current_task` to reflect what was done, append to `recent_decisions`
2. Update `session_metadata.skills_invoked` if a skill was used
3. If environment discovery ran this session, ensure `capabilities.json` is written to `~/.codex/ftm-state/blackboard/capabilities.json` with the current snapshot (schema: `capabilities.schema.json` in the same directory)
...
- config: `~/.codex/ftm-config.yml` | optional | approval_mode, execution preferences
- reference: `~/.codex/skills/ftm-mind/references/mcp-inventory.md` | required | MCP capability routing table
- reference: `~/.codex/ftm-state/blackboard/context.json` | optional | session state and preferences
- reference: `~/.codex/ftm-state/blackboard/experiences/index.json` | optional | experience retrieval index
- reference: `~/.codex/ftm-state/blackboard/patterns.json` | optional | promoted patterns for orientation
