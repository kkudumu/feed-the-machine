...
**Steps**:
1. Run `bash ~/.codex/skills/ftm-upgrade/scripts/check-version.sh`
2. Parse the output:
...
Run:
```bash
bash ~/.codex/skills/ftm-upgrade/scripts/check-version.sh
```
...
Read the current version from `~/.codex/skills/ftm-version.txt` (if it exists) and include it in the message:
...
Run:
```bash
bash ~/.codex/skills/ftm-upgrade/scripts/upgrade.sh
```

Stream the output to the user as it runs.

On success, confirm:
> Upgrade complete. FTM skills updated to <version>. Restart Codex to load new skills.

On failure (non-zero exit or error lines in output):
> Upgrade failed. See error above. If the problem persists, try: `bash ~/.codex/skills/ftm-upgrade/scripts/upgrade.sh` manually.
...
1. Run `bash ~/.codex/skills/ftm-upgrade/scripts/check-version.sh`
2. Also read `~/.codex/skills/ftm-version.txt` for the installed version
...
```
No version file found at ~/.codex/skills/ftm-version.txt.
...
| Script | Purpose |
|---|---|
| `~/.codex/skills/ftm-upgrade/scripts/check-version.sh` | Query GitHub for latest release, cache result 60 min |
| `~/.codex/skills/ftm-upgrade/scripts/upgrade.sh` | Download and install latest release |
...
- level: high_write
- scope: downloads and overwrites skill files in ~/.codex/skills/ on upgrade; changes affect all ftm skill behavior going forward; irreversible without restoring previous version from backup or git
- rollback: restore from ~/.codex/skills/ backup if one was made before upgrade; or reinstall specific version by downloading an older release tarball
