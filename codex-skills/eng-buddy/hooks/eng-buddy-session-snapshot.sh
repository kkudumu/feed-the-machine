...
# We fire on SessionEnd since Codex doesn't expose /new or /reset as hook events.
#
# Filtering:
...
#
# OUTPUT: ~/.codex/eng-buddy/sessions/
# MINIMUM: 3 meaningful messages required to write a snapshot (skip trivial sessions)
#
# FILES:
#   ~/.codex/eng-buddy/.session-active    - session gate (read only, not modified)
#   ~/.codex/eng-buddy/sessions/          - snapshot output directory

SESSION_ACTIVE="$CODEX_HOME/eng-buddy/.session-active"
SESSIONS_DIR="$CODEX_HOME/eng-buddy/sessions"
MAX_MESSAGES=15
MIN_MESSAGES=3
