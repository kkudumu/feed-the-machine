...
#
# Detects when Codex has just compacted the session (context was compressed).
...
#
# FILES:
#   ~/.codex/eng-buddy/.session-active         - session gate
#   ~/.codex/eng-buddy/.session-state          - JSON: {session_id, line_count}

SESSION_ACTIVE="$CODEX_HOME/eng-buddy/.session-active"
SESSION_STATE_FILE="$CODEX_HOME/eng-buddy/.session-state"
...
echo "1. Read ~/.codex/eng-buddy/daily/${TODAY}.md (today's session log)"
echo "2. Read ~/.codex/eng-buddy/tasks/active-tasks.md (current task state)"
echo "3. Read ~/.codex/eng-buddy/dependencies/active-blockers.md (active blockers)"
