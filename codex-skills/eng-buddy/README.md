...
A Codex skill + local web dashboard that turns your `~/.codex/` directory into an intelligent engineering operations center. Background pollers watch Gmail, Slack, Jira, Google Calendar, and Freshservice — surfacing actionable cards you can approve, hold, refine, or execute as full Claude sessions. A built-in planner decomposes cards into executable plans, and a learning engine captures patterns from every interaction.
...
cd ~/.codex/eng-buddy/dashboard
./start.sh
# Opens at http://localhost:7777

# 4. Invoke the skill in Codex
$eng-buddy
```
...
```
~/.codex/skills/eng-buddy/          # Source repo (kkudumu/eng-buddy)
...
├── hooks/                           # Codex hook scripts (7 hooks)
...
**Runtime directory** (`~/.codex/eng-buddy/`):
```
...
Seven Codex hooks provide session automation. All hooks are session-gated — they only activate during `$eng-buddy` sessions.
...
cp ~/.codex/eng-buddy/email-watches.md.example ~/.codex/eng-buddy/email-watches.md
```
...
### Jira Poller Setup
Requires Codex CLI + [Atlassian MCP server](https://github.com/sooperset/mcp-atlassian) configured in `.claude.json`. The poller calls `claude --dangerously-skip-permissions --print` to query via MCP.
...
<dict>
  ...
  <array>
    ...
    <string>${HOME}/.codex/eng-buddy/bin/gmail-poller.py</string>
  </array>
  <key>StartInterval</key><integer>600</integer>
  <key>StandardOutPath</key><string>${HOME}/.codex/eng-buddy/gmail-poller.log</string>
  <key>StandardErrorPath</key><string>${HOME}/.codex/eng-buddy/gmail-poller.log</string>
</dict>
</plist>
EOF
...
```bash
bash ~/.codex/eng-buddy/bin/start-pollers.sh
```
...
# Check logs
tail -f ~/.codex/eng-buddy/gmail-poller.log
tail -f ~/.codex/eng-buddy/slack-poller.log
tail -f ~/.codex/eng-buddy/jira-poller.log
tail -f ~/.codex/eng-buddy/calendar-poller.log
```
