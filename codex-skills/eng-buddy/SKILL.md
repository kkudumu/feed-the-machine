---
name: eng-buddy
description: Your personal IT systems engineering assistant with comprehensive self-tracking. Organizes tasks, analyzes patterns, generates narrative insights, and tracks everything about your working style. Data goblin mode enabled.
---
...
- Use Bash: bash ~/.codex/skills/eng-buddy/bin/install-hooks.sh
- This syncs hooks to parent + child locations (`~/.codex/hooks`, `~/.codex/skills/eng-buddy/hooks`, `~/.codex/eng-buddy/hooks`),
  patches `~/.codex/settings.json`, and ensures learning-engine runtime files are present
- Then use Bash: ~/.codex/hooks/eng-buddy-session-manager.sh start
...
- Use Bash: python3 ~/.codex/skills/eng-buddy/bin/brain.py --tasks --task-json
- Parse the JSON output to get all active tasks from tasks.db
- For each pending or in-progress task: load it from `tasks.db` and surface it in a numbered task summary in your response
...
- Use Bash (run in background): ~/.codex/eng-buddy/dashboard/start.sh --background
...
- If start.sh outputs "TIMEOUT", warn the user: "Dashboard failed to start — check ~/.codex/eng-buddy/dashboard.log"
- Tell the user: "Dashboard is live at http://127.0.0.1:7777"

STEP 0.7: Start pollers (MUST DO ON EVERY INVOCATION)
- Use Bash: bash ~/.codex/skills/eng-buddy/bin/start-pollers.sh
...
- Use Bash: ls -la ~/.codex/eng-buddy/ 2>/dev/null || echo "WORKSPACE_DOES_NOT_EXIST"
...
ELSE IF directory exists:
  ...
  → Count daily files: ls -1 ~/.codex/eng-buddy/daily/*.md 2>/dev/null | wc -l
...
1. Use Bash to create directory structure: `mkdir -p ~/.codex/eng-buddy/{daily,weekly,monthly,knowledge,patterns,incidents,dependencies,capacity,stakeholders/status-updates,sessions,archive}`
...
1. **Check and restore task state** (CRITICAL - rely on `tasks.db`, not transient UI task lists):
...
4. Check if today's daily file exists: `ls ~/.codex/eng-buddy/daily/$(date +%Y-%m-%d).md`
...
- Always use `~/.codex/eng-buddy/` as base path
- Expand to full path when needed: `$CODEX_HOME/eng-buddy/`
...
2. Write the file to `~/.codex/eng-buddy/drafts/` using the Write tool — this happens BEFORE anything else
...
`$CODEX_HOME/eng-buddy/drafts/nik-structure-licensing-2026-02-19.md`
...
**File Structure:** `~/.codex/eng-buddy/`
```
...
**Execution**: When a new ticket matches an approved playbook, the dashboard shows a "Ready to Execute" card with the pre-filled step list. Type your approval command and eng-buddy dispatches a Codex session in your terminal to execute it.
...
- CLI: `python3 ~/.codex/eng-buddy/bin/brain.py --playbook-list`
...
```bash
echo "ITWORK2-1234" > ~/.codex/eng-buddy/.active-trace-id
```

eng-buddy hooks will automatically record tool calls against this trace. Clear it when done:

```bash
rm ~/.codex/eng-buddy/.active-trace-id
```

#### Tool Registry

Playbook steps bind to tools via the registry at `~/.codex/eng-buddy/playbooks/tool-registry/`. Each tool has:
...
Each playbook is a JSON file in `~/.codex/eng-buddy/playbooks/` with this structure:
...
**Problem**: transient in-session task UIs do not persist across conversations.

**Solution**: Use `~/.codex/eng-buddy/tasks.db` (SQLite) as the single source of truth for all task state. The `tasks_db.py` module provides all CRUD operations. The old `active-tasks.md` is kept as a read-only archive but is NO LONGER the authoritative source.

**On EVERY task change**:
1. Update `tasks.db` immediately via `brain.py` CLI:
   - New task: `python3 ~/.codex/skills/eng-buddy/bin/brain.py --task-add --title "X" --priority P --jira-key K`
   - Status change: `python3 ~/.codex/skills/eng-buddy/bin/brain.py --task-update N --status S`
   - Priority change: `python3 ~/.codex/skills/eng-buddy/bin/brain.py --task-update N --priority P`
   - Defer: `python3 ~/.codex/skills/eng-buddy/bin/brain.py --task-update N --deferred-until "YYYY-MM-DD"`
3. Update daily log to match

**On EVERY task read**:
1. Run `python3 ~/.codex/skills/eng-buddy/bin/brain.py --tasks` for a quick table view
...
**Why this matters**:
- Task numbers don't show in Codex UI by default
...
**Example task snapshot format**:
```
#5 - Redesign Jira Project Access workflow (DUE 2PM)
Status: in_progress
Priority: P1
Jira: ITWORK2-1234
Summary: waiting on access model decision before rollout
```
...
✅ Your workspace is ready at `~/.codex/eng-buddy/`
...
- The SessionEnd hook will automatically run: `~/.codex/hooks/eng-buddy-session-manager.sh stop`
...
No setup needed beyond having Codex. Just invoke `$eng-buddy`.
...
|--------|---------|--------------|
| `eng-buddy-session-manager.sh` | Called by SKILL.md STEP 0 | Activates/deactivates the session marker. `start` creates `~/.codex/eng-buddy/.session-active`, `stop` removes it. Gates all other hooks. |
...
| `eng-buddy-post-compaction.sh` | `UserPromptSubmit` (every message) | **Post-compaction context restoration.** Detects when Codex has compacted the session (same session ID, fewer JSONL lines). After compaction, Claude loses in-session state — this hook injects a re-initialization prompt telling Claude to reload its daily log, task state, and blockers before responding. Only fires during active eng-buddy sessions. |
...
1. Append unsaved session progress to ~/.codex/eng-buddy/daily/YYYY-MM-DD.md
...
1. Read ~/.codex/eng-buddy/daily/YYYY-MM-DD.md (today's session log)
2. Run brain.py --tasks --task-json to reload current task state from tasks.db
3. Read ~/.codex/eng-buddy/dependencies/active-blockers.md (active blockers)
...
```
~/.codex/eng-buddy/sessions/2026-02-25T14-30-redesign-jira-project-access-workflow.md
```
...
- Read ~/.codex/eng-buddy/dependencies/active-blockers.md — any aging blockers to escalate?
...
`~/.codex/eng-buddy/HEARTBEAT.md` is a user-maintained file for persistent alerts. Lines starting with `#` and blank lines are ignored. Add actionable items here that you want surfaced at every 30-minute check-in — delete them when no longer relevant.

**How completion capture + category expansion works:**

`eng-buddy-learning-capture.sh` runs on `PostToolUse`, so it sees completed `Write/Edit/Bash/MCP` actions as they happen. It records each eligible completion in `~/.codex/eng-buddy/inbox.db` (`learning_events`) and maps it to a learning category in `learning_categories`.
...
```bash
python3 ~/.codex/eng-buddy/bin/brain.py \
...
bash ~/.codex/skills/eng-buddy/bin/install-hooks.sh
...
mkdir -p "$HOOKS_DIR"
mkdir -p ~/.codex/eng-buddy/sessions
cp ~/.codex/skills/eng-buddy/hooks/eng-buddy-auto-log.sh "$HOOKS_DIR/"
cp ~/.codex/skills/eng-buddy/hooks/eng-buddy-learning-capture.sh "$HOOKS_DIR/"
cp ~/.codex/skills/eng-buddy/hooks/eng-buddy-pre-compaction.sh "$HOOKS_DIR/"
cp ~/.codex/skills/eng-buddy/hooks/eng-buddy-post-compaction.sh "$HOOKS_DIR/"
cp ~/.codex/skills/eng-buddy/hooks/eng-buddy-session-snapshot.sh "$HOOKS_DIR/"
cp ~/.codex/skills/eng-buddy/hooks/eng-buddy-session-end.sh "$HOOKS_DIR/"
cp ~/.codex/skills/eng-buddy/hooks/eng-buddy-session-manager.sh "$HOOKS_DIR/"
chmod +x "$HOOKS_DIR"$eng-buddy-*.sh

# 3. Wire UserPromptSubmit, PostToolUse, and SessionEnd in settings.json
# Location: $CODEX_HOME/settings.json or ~/.codex/settings.json
```
...
The Slack poller runs in the background every 10 minutes, pulls your DMs, private channel messages, and @mentions, detects task-signal messages, and writes them to `~/.codex/eng-buddy/task-inbox.md`.
...
mkdir -p ~/.codex/eng-buddy/bin
cp ~/.codex/skills/eng-buddy/bin/slack-poller.py ~/.codex/eng-buddy/bin/
chmod +x ~/.codex/eng-buddy/bin/slack-poller.py

# 2. Set your token in the script
sed -i '' 's|YOUR_SLACK_USER_TOKEN|xoxp-YOUR-ACTUAL-TOKEN|g' \
  ~/.codex/eng-buddy/bin/slack-poller.py

# 3. Copy the launchd plist and set your token
cp ~/.codex/skills/eng-buddy/bin/com.engbuddy.slackpoller.plist \
...
# 5. Test it
python3 ~/.codex/eng-buddy/bin/slack-poller.py
# Expected: [HH:MM] No new messages  (or a list of messages)
```

**Logs:** `~/.codex/eng-buddy/slack-poller.log`
...
mkdir -p ~/.codex/eng-buddy/bin
cp ~/.codex/skills/eng-buddy/bin/gmail-poller.py ~/.codex/eng-buddy/bin/
chmod +x ~/.codex/eng-buddy/bin/gmail-poller.py

# 2. Create your email-watches.md (copy template, edit as needed)
cp ~/.codex/skills/eng-buddy/email-watches.md.template \
   ~/.codex/eng-buddy/email-watches.md

# 3. Copy the launchd plist
cp ~/.codex/skills/eng-buddy/bin/com.engbuddy.gmailpoller.plist \
...
# 4. Test it
python3 ~/.codex/eng-buddy/bin/gmail-poller.py
...
**Logs:** `~/.codex/eng-buddy/gmail-poller.log`
...
# Hooks
mkdir -p "$HOOKS_DIR"
cp "$CODEX_HOME/skills/eng-buddy/hooks/"*.sh "$HOOKS_DIR/"
chmod +x "$HOOKS_DIR"$eng-buddy-*.sh
...
# Slack poller
cp "$CODEX_HOME/skills/eng-buddy/bin/slack-poller.py" ~/.codex/eng-buddy/bin/
sed -i '' "s|YOUR_SLACK_USER_TOKEN|$SLACK_TOKEN|g" ~/.codex/eng-buddy/bin/slack-poller.py
chmod +x ~/.codex/eng-buddy/bin/slack-poller.py

cp "$CODEX_HOME/skills/eng-buddy/bin/com.engbuddy.slackpoller.plist" \
   ~/Library/LaunchAgents/com.engbuddy.slackpoller.plist
...
cp "$CODEX_HOME/skills/eng-buddy/bin/gmail-poller.py" ~/.codex/eng-buddy/bin/
chmod +x ~/.codex/eng-buddy/bin/gmail-poller.py
cp "$CODEX_HOME/skills/eng-buddy/email-watches.md.template" \
   ~/.codex/eng-buddy/email-watches.md

cp "$CODEX_HOME/skills/eng-buddy/bin/com.engbuddy.gmailpoller.plist" \
   ~/Library/LaunchAgents/com.engbuddy.gmailpoller.plist
sed -i '' "s|/opt/homebrew/bin/python3|$PYTHON|g" \
  ~/Library/LaunchAgents/com.engbuddy.gmailpoller.plist
launchctl load ~/Library/LaunchAgents/com.engbuddy.gmailpoller.plist
...
echo "Edit ~/.codex/eng-buddy/email-watches.md to add your email watches."
```
