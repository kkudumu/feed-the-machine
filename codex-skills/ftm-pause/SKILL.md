...
```bash
mkdir -p ~/.codex/ftm-state
```

Write the state file to `~/.codex/ftm-state/STATE.md`:
...
## Artifacts
- ~/.codex/plans/ — no plan generated yet (still in Phase 2)
...
```
Session saved to ~/.codex/ftm-state/STATE.md
...
- reference: `~/.codex/ftm-state/STATE.md` | optional | existing state file to overwrite
- reference: `~/.codex/ftm-pause/references/protocols/SKILL-RESTORE-PROTOCOLS.md` | required | per-skill capture field specifications
- reference: `~/.codex/ftm-pause/references/protocols/VALIDATION.md` | required | pre-write and post-write validation checklist
- tool: `git` | optional | git branch and commit hash capture for state file

## Risk

- level: low_write
- scope: writes ~/.codex/ftm-state/STATE.md only; does not modify project source files or blackboard experiences; overwrites existing STATE.md without backup
...
- condition: ~/.codex/ftm-state/ directory doesn't exist | action: create directory before writing STATE.md
