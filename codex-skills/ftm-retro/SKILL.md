...
2. If no `PROGRESS.md` exists, check `~/.codex/ftm-retros/` for the most recent `.md` file and ask the user which execution they want to review.
...
Before writing anything, check whether any `.md` files exist in `~/.codex/ftm-retros/`. If they do, read them all. You will use them for the Pattern Analysis section.

### Step 4: Write the report

Save to: `~/.codex/ftm-retros/{plan-slug}-{YYYY-MM-DD}.md`
...
{Only include this section if past retros exist in ~/.codex/ftm-retros/}
...
```
Retro saved: ~/.codex/ftm-retros/{filename}
...
Write a structured experience entry to `~/.codex/ftm-state/blackboard/experiences/YYYY-MM-DD_task-slug.json` following the schema in blackboard-schema.md.
...
- reference: `~/.codex/ftm-retros/` | optional | prior retro files for pattern analysis
...
- reference: `~/.codex/ftm-state/blackboard/experiences/index.json` | optional | experience inventory for micro-reflection mode
- reference: `~/.codex/ftm-state/blackboard/patterns.json` | optional | pattern registry for promotion and decay

## Risk

- level: low_write
- scope: writes retro report to ~/.codex/ftm-retros/; writes experience files to blackboard; promotes patterns to patterns.json; does not modify project source files
...
- condition: PROGRESS.md not found and manual mode | action: check ~/.codex/ftm-retros/ for most recent .md file; ask user which execution to review if multiple found
