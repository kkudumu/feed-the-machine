...
1. Update the blackboard context: read `~/.codex/ftm-state/blackboard/context.json`, set `current_task` to reflect the incoming request, append to `session_metadata.skills_invoked`, write back.
...
### Legacy Fallback
If ftm-mind fails (errors, timeouts, no actionable output) AND `legacy_router_fallback` is `true` in `~/.codex/ftm-config.yml`, fall back to keyword matching:
...
- config: `~/.codex/ftm-config.yml` | optional | legacy_router_fallback setting
- reference: `~/.codex/ftm-state/blackboard/context.json` | optional | session state for blackboard update on routing
