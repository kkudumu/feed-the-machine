...
7. Phase 3 — runtime wiring via panda-browse (if prerequisites met)
...
**Prerequisites (all required):** panda-browse binary at `$HOME/.claude/skills/panda-browse/bin/panda-browse` + dev server running + wiring contracts include `route_path` entries.
...
1. Update `~/.claude/panda-state/blackboard/context.json` — set current_task complete, append to recent_decisions (cap 10), update session_metadata
2. Write experience file to `~/.claude/panda-state/blackboard/experiences/YYYY-MM-DD_task-slug.json` — findings count, fix count, dimensions fired, manual interventions
