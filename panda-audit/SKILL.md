...
**What:** A wiring contract is a YAML block in a plan task that declares the expected wiring for code produced by that task. It tells panda-audit exactly what to verify — instead of guessing, the audit checks specific expectations.
...
**How panda-audit checks contracts:**
...
1. The panda-browse binary exists at `$HOME/.claude/skills/panda-browse/bin/panda-browse`
...
If panda-browse is not installed or the dev server is not running:
...
When invoked (manually via `/panda-audit` or automatically post-task):
...
7. Run Phase 3 (runtime wiring via panda-browse, if prerequisites met)
