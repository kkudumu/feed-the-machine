...
Before running panda-audit, verify these four checks for every task:
...
5. **Visual smoke test (optional)** — If the project has a running dev server (detected via `lsof -i :3000` or `lsof -i :5173` or configured in plan metadata as `dev_server_url`), run:
   ...
   **Graceful degradation**: If panda-browse binary is not installed, skip visual checks with a note: "Visual smoke test skipped — panda-browse not installed." Do not fail the task.
...
- Missing INTENT.md entries → add them (use panda-intent format)
- Missing diagram nodes → add them (use panda-diagram format)
- Regression failures → investigate and fix before continuing

## Running panda-audit

Use `review` model from panda-config when spawning audit agents.

**Invoke panda-audit** scoped to the files the task modified (check the agent's commits). If the task has a `Wiring:` contract in the plan, pass it to panda-audit for contract checking. Run all three layers: knip static analysis → adversarial audit → auto-fix.
