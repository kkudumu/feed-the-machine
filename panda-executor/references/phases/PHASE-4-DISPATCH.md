...
Pass the `execution` model from panda-config as the `model` parameter when spawning dispatch agents. If the profile specifies `inherit`, omit the `model` parameter.
