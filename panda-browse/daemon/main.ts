#!/usr/bin/env bun
/**
 * main.ts — Unified entry point for the compiled panda-browse binary.
 *
 * When invoked as:
 *   panda-browse --server        → runs the HTTP daemon (server.ts logic)
 *   panda-browse <command> ...   → runs the CLI client (cli.ts logic)
 */
...
async function ensureDaemon(): Promise<DaemonState> {
  ...
  console.error("[panda-browse] Starting daemon...");
  ...
  while (Date.now() < timeout) {
    ...
    if (state && isProcessAlive(state.pid)) {
      console.error(`[panda-browse] Daemon started on port ${state.port}`);
      return state;
    }
  }

  throw new Error(
    "Daemon failed to start within 10 seconds. Check ~/.panda-browse/logs for details."
  );
}
...
function parseArgs(argv: string[]): ParsedCommand {
  ...
  // For compiled: process.argv = ["/path/to/panda-browse", "goto", "url"]
  ...
  switch (command) {
    case "goto": {
      if (!args[1]) { console.error("Usage: panda-browse goto <url>"); process.exit(1); }
    ...
    case "click": {
      if (!args[1]) { console.error("Usage: panda-browse click <@ref>"); process.exit(1); }
      return { command: "click", args: { ref: args[1] } };
    }
    case "fill": {
      if (!args[1] || args[2] === undefined) { console.error("Usage: panda-browse fill <@ref> <value>"); process.exit(1); }
      return { command: "fill", args: { ref: args[1], value: args.slice(2).join(" ") } };
    }
    case "press": {
      if (!args[1]) { console.error("Usage: panda-browse press <key>"); process.exit(1); }
    ...
    case "chain": {
      if (!args[1]) { console.error("Usage: panda-browse chain <json-array>"); process.exit(1); }
...
function printUsage(): void {
  console.error(`
panda-browse - Headless browser daemon for Claude Code agents

USAGE:
  panda-browse <command> [args]

COMMANDS:
  goto <url>                 Navigate to URL
  snapshot [-i]              Get ARIA tree snapshot. -i = interactive elements only
  screenshot [--path <file>] Take screenshot (default: ~/.panda-browse/screenshots/)
...
if (firstArg === "--server") {
  runServer().catch((err) => {
    console.error("[panda-browse] Fatal server error:", err instanceof Error ? err.message : String(err));
    process.exit(1);
  });
} else {
  runCli().catch((err) => {
