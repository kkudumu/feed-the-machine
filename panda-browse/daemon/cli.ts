...
function parseArgs(argv: string[]): ParsedCommand {
  ...
  switch (command) {
    case "goto": {
      if (!args[1]) {
        console.error("Usage: panda-browse goto <url>");
        process.exit(1);
      }
    ...
    case "click": {
      if (!args[1]) {
        console.error("Usage: panda-browse click <@ref>");
        process.exit(1);
      }
    ...
    case "fill": {
      if (!args[1] || args[2] === undefined) {
        console.error("Usage: panda-browse fill <@ref> <value>");
        process.exit(1);
      }
    ...
    case "press": {
      if (!args[1]) {
        console.error("Usage: panda-browse press <key>");
        process.exit(1);
      }
    ...
    case "chain": {
      if (!args[1]) {
        console.error("Usage: panda-browse chain <json-array>");
        process.exit(1);
      }
