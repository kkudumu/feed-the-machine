...
### Listens To
(none — council is invoked explicitly by the user or by panda-executor when an INTENT.md conflict requires arbitration)
...
2. **Auto-invoked**: Another skill (typically panda-executor) invokes the council with a pre-framed conflict payload. Skip Step 0 — the problem is already framed.
...
- When auto-invoked by panda-executor — always proceed (the executor already determined a council is needed)
