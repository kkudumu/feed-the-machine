---
name: panda-codex-gate
description: Codex CLI integration gate for adversarial code validation. Invokes codex exec --yolo --ephemeral with gpt-5.4 to form test scenarios, review code quality, fix failures, and enforce STYLE.md patterns. Use when panda-executor needs Codex validation at wave boundaries or task completion, or when user says "codex gate", "run codex", "validate with codex". Not for direct user interaction — primarily auto-invoked by panda-executor.
---
...
This skill is the integration layer between panda-executor and the Codex CLI. It constructs adversarial validation prompts, runs them through `codex exec`, captures structured output, and returns results to the calling skill. It does not interact with the user directly — it is invoked at wave boundaries or task completion.

---

## Inputs

Expect these inputs from the calling skill (panda-executor). If any are missing, ask for them before proceeding.
...
Return the full structured summary to the calling skill (panda-executor) in this exact format:
...
Pass the structured result back to panda-executor. Do not post to any external system, do not notify the user directly unless panda-executor explicitly delegates that to this skill.

If Status is FAIL or PASS_WITH_FIXES, include the full "Remaining Issues" and "INTENT.md Conflicts" sections so panda-executor can decide whether to retry, escalate, or continue to the next wave.
