...
Structured retrospective system for panda-executor plans. Scores execution across 5 evidence-based dimensions, surfaces bottlenecks with specifics, and builds a cumulative pattern library that makes each future execution smarter.
...
### Mode 1: Auto-triggered by panda-executor (Phase 6.5)
...
2. If no `PROGRESS.md` exists, check `~/.claude/panda-retros/` for the most recent `.md` file and ask the user which execution they want to review.
...
What percentage of tasks passed panda-audit on the first attempt?
...
What percentage of waves passed the panda-codex-gate on the first attempt?
...
Examples:
- "Panda Ecosystem Expansion" → `panda-ecosystem-expansion`
- "Fix Auth Bug + Rate Limiting" → `fix-auth-bug-rate-limiting`
- "v2.0 API Refactor" → `v20-api-refactor`

### Step 3: Check for past retros

Before writing anything, check whether any `.md` files exist in `~/.claude/panda-retros/`. If they do, read them all. You will use them for the Pattern Analysis section.

### Step 4: Write the report

Save to: `~/.claude/panda-retros/{plan-slug}-{YYYY-MM-DD}.md`
...
Example format:
- **panda-audit Phase 1 (knip)** repeated full project analysis for each task in wave 3, even though tasks only touched 2–3 files each. Added ~40s × 5 tasks = ~3.5 minutes of unnecessary scanning.
...
Examples:
1. **Cache knip results within a wave** — panda-audit — In Phase 1, check whether knip results are already cached for the current wave (via a temp file at `/tmp/panda-knip-cache-{wave-id}.json`). Only re-run knip if the cache is missing or if the files changed by this task differ from cached scope. Expected: 3× speedup for panda-audit on large projects with many tasks per wave.
2. **Dispatch Instrumentor and Researcher in parallel** — panda-debug — These two agents have no shared state and currently run sequentially. Dispatch them simultaneously. Expected: ~40% reduction in panda-debug total runtime.
3. **Add tsconfig.paths to agent context for TypeScript projects** — panda-executor — When generating agent prompts for TypeScript tasks, include the relevant `paths` aliases from `tsconfig.json`. Expected: eliminates the import-alias regeneration loop that caused 3 retries on Task 7.

## Pattern Analysis

{Only include this section if past retros exist in ~/.claude/panda-retros/}
...
Do not write "the execution felt smooth" or "agents seemed efficient." Write "0 manual interventions were required and all errors were caught and auto-resolved by panda-audit Phase 2." The report is read by future executions that need to calibrate behavior, not by humans looking for encouragement.
...
```
Retro saved: ~/.claude/panda-retros/{filename}
...
Write a structured experience entry to `~/.claude/panda-state/blackboard/experiences/YYYY-MM-DD_task-slug.json` following the schema in blackboard-schema.md.
