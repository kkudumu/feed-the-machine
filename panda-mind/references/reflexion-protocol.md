...
**Location**: `~/.claude/skills/panda-mind/references/reflexion-protocol.md`
**Purpose**: Defines how panda-mind stores and reuses execution experience through the Reflexion pattern — a lightweight verbal reinforcement learning loop that improves behavior across sessions without fine-tuning.
...
When panda-mind enters the Orient phase of the OODA loop for a new task:
...
```json
{
  ...
  "capabilities_used": ["panda-executor", "mcp__mcp-atlassian-personal__jira_get_issue", "backend-architect"],
...
Written to: `~/.claude/panda-state/blackboard/experiences/YYYY-MM-DD_task-slug.json`
...
| Experience file location | `~/.claude/panda-state/blackboard/experiences/YYYY-MM-DD_task-slug.json` |
