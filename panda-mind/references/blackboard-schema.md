...
**Location**: `~/.claude/panda-state/blackboard/`
**Purpose**: Shared persistent memory for the panda-mind unified intelligence system. Skills read from and write to these files using the Read and Write tools directly — no code libraries, no APIs.
...
| `STATE.md` | Pause/resume snapshots (panda-pause / panda-resume) | Anything not related to pausing a session mid-stream |
| `PROGRESS.md` | Executor wave progress during multi-agent runs | Session state, historical learnings |
| `DEBUG.md` | Debug session traces from panda-debug | Non-debug information |
...
**Path**: `~/.claude/panda-state/blackboard/patterns.json`
...
**Directory**: `~/.claude/panda-state/blackboard/experiences/`
...
- **`capabilities_used`**: Include skill names (e.g. `panda-executor`, `panda-debug`), MCP server names (e.g. `mcp__mcp-atlassian-personal__jira_get_issue`), and agent types (e.g. `backend-architect`).
...
Multiple executor agents may run in parallel during a panda-executor wave. To prevent index corruption:
