...
Save the full state of any active panda skill session to disk so it can be resumed in a new conversation with zero context loss.
...
Capture every field listed for the detected skill. Do not omit fields because a phase hasn't been reached yet — record those as "not started" or "N/A" so panda-resume knows the session stopped before that phase.
...
Write the state file to `~/.claude/panda-state/STATE.md`. Required structure:
...
## Next Step
[Specific and actionable: what panda-resume does first, what the user needs to respond to,
